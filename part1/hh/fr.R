...
VALUES <- cbind(e_rest, c, g_leak, e_leak, g_na, e_na, g_k, e_k, ##i_ext,
                dt, t, nt) %>% data.frame()
...
i_ext_start <- 6.0
i_ext_end <- 20.0
i_ext_roop <- seq(i_ext_start, i_ext_end, by = 0.1)
DF <- data.frame(i_ext = numeric(length(i_ext_roop)), ns = numeric(length(i_ext_roop)))
...
for(j in i_ext_roop){
  ...
  for(i in 1:nt) {
    ...
    ## count the spike by comparing this v and last(old) v
    ns <-  ifelse(
      v > 0 & v_old < 0,
      ns + 1,
      ns
  ...
  DF[10*i_ext - 10*i_ext_start + 1,] <- c(j, ns)
  ### im waiting for 'for' you but tell me if you are not stacking
  print(j)
  
  ### writing the spike rates in each conditions
...
fr_plot <- DF %>% ggplot(
  .,
  aes(x = i_ext, y = rate) 
) + geom_line() + xlim(i_ext_start, i_ext_end)
ggsave("part1/hh/fr_plot.png", fr_plot, dpi = 320)
fr_plot
