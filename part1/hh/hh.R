...
VALUES <- cbind(e_rest, c, g_leak, e_leak, g_na, e_na, g_k, e_k, i_ext, dt, t, nt) %>% 
  data.frame()
...
DF <- data.frame(t = numeric(VALUES$nt), v = numeric(VALUES$nt), m = numeric(VALUES$nt),
                 h = numeric(VALUES$nt), n = numeric(VALUES$nt))

names(DF) <- c("t", "v", "m", "h", "n")
...
for(i in 1:nt) {
  ...
  dhdt1 <- dhdt(v, h)
  dndt1 <- dndt(v, n)
  dvdt1 <- dvdt(v, m, h, n, VALUES$i_ext)
...
ggsave("part1/hh/hh_spike.png", hh_spike, dpi = 320)
